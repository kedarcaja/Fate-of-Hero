...
public class Slot : MonoBehaviour, IPointerClickHandler
{
    ...
    private void UseItem() {
        if (!IsEmpty)
        {
            ...
            {
                if (CurrentItem.Item.BuyPrice <= IPlayer.Instance.Gold && IPlayer.Instance.inventory.AddItem(CurrentItem))
                {
                    IPlayer.Instance.Gold -= CurrentItem.Item.BuyPrice;
                }
            }
            else if (VendorInvetory.Instance.IsOpen)
            {
                IPlayer.Instance.Gold += CurrentItem.Item.SellPrice;
                RemoveItem();
            }
            else if (clickAble)
            {
